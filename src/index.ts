import { Command, UndoRedoInterface, Queue } from "./index.d"
class UndoRedo implements UndoRedoInterface {
    state = {
        current: -1,//前进后索引
    ...
    registry(command: Command) {
        ...
        this.state.commands[command.name] = (...args) => {
            ...
            //     console.log("redo",this.state.queue.length - 1 ,this.state.current)
            ...
            //        console.log("undo")
            //     if(this.state.queue[this.state.current+1]){
            //          typeof this.changeEvent === 'function' && this.changeEvent();
            //     }
            ...
            execute: () => {
                return {
                    undo: () => {
                        ...
                        console.log("this.state.current undo", this.state.current, this.state.queue, item)
